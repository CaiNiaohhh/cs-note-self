...
- 异步处理：就是说生产者生存数据之后，消费者不用立即接收，可以等到有需要再接收
- 应用解耦：假设有订单系统和库存系统，假设库存系统无法访问，则订单将会一直失败，这时候可以考虑使用消息队列，
  订单系统：用户下单后，订单系统完成持久化处理，将消息写入消息队列，返回用户订单下单成功 
  库存系统：订阅下单的消息，采用拉/推的方式，获取下单信息，库存系统根据下单信息，进行库存操作 
  假如：在下单时库存系统不能正常使用。也不影响正常下单，因为下单后，订单系统写入消息队列就不再关心其他的后续操作了。实现订单系统与库存系统的应用解耦。
  这样就保证了说用户下单是无障碍的，至于库存系统里面的可以灵活处理，一直没有库存在通知用户
- 流量削锋：一般用于秒杀场景，用户的请求，服务器接收后， 首先写入消息队列。假如消息队列长度超过最大数量，
  则直接抛弃用户请求或跳转到错误页面。 秒杀业务根据消息队列中的请求信息，再做后续处理。
- 日志处理：日志采集客户端，负责日志数据采集，定时写受写入Kafka队列；Kafka消息队列，负责日志数据的接收，
  存储和转发；日志处理应用：订阅并消费kafka队列中的日志数据。
...
- Kafka 官方文档： https://kafka.apache.org/documentation/
  - 中文：https://kafka.apachecn.org/intro.html
